  ...
  export default {
    ...
    methods: {
      ...
      setImage(obj) {
        if(obj.type === 'blur') {
          let flag = true
          this.canvas.getActiveObject().filters.forEach(item => {
            if(item.blur) {
              item.blur = obj.value
              flag = false
            }
          ...
          this.canvas.getActiveObject().applyFilters()
        } else if(obj.type === 'contrast') {
          let flag = true
          this.canvas.getActiveObject().filters.forEach(item => {
            if(item.contrast) {
              item.contrast = obj.value
              flag = false
            }
          })
          if(flag) {
      ...
      designAlign(type) {
        ...
        if(type === 'right') {
          object.set({
            left: this.canvas.width - (object.width * object.scaleX)
          })
        }
        if(type === 'centerH') {
          object.centerH()
        ...
        if(type === 'bottom') {
          object.set({
            top: this.canvas.height - (object.height * object.scaleY)
          })
        }
        object.setCoords()
        this.canvas.renderAll()
