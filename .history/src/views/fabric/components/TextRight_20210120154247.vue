    ...
    <div class="content">
      ...
      <div class="set-fontgradient" v-if="gradient">
        ...
        <div class="showColor clearfix">
          <div class="choiceColor" @click="showGradientFirstChrome = !showGradientFirstChrome" :style="'background:'+gradient.colorStops[0].color"></div>
          <div class="choiceColor" @click="showGradientLastChrome = !showGradientLastChrome" :style="'background:'+gradient.colorStops[1].color"></div>
        </div>
        <div class="con" v-if="showGradientFirstChrome || showGradientLastChrome">
          <Chrome v-model="gradient.colorStops[0].color" @input="selectGradientFirstColor" v-if="showGradientFirstChrome"/>
          <Chrome v-model="gradient.colorStops[1].color" @input="selectGradientLastColor" v-if="showGradientLastChrome"/>
        </div>
        <p style="padding-top:10px;">渐变类型</p>
        <el-select v-model="gradient.type" placeholder="请选择" @change="selectGradient">
          <el-option label="线性渐变" value="linear"></el-option>
          <el-option label="径向渐变" value="radial"></el-option>
        </el-select>
