<template>
  <div style="height:100%;">
    <Header @preview="preview" @download="download" @save="save" @undo="undo" @redo="redo"/>
    <AsideLeft
      @setFont="setFont"
...
export default {
  ...
  methods: {
    ...
    undo() {
      ...
      //   this.undoRedoArr.push(this.canvas._objects.pop())
      //   this.canvas.renderAll()
      // }
      // let currentTarget = this.canvas.getActiveObject()
      console.log(this.config.undoFinishedStatus)
    ...
    redo() {
      console.log('redo')
      // 恢复
      // if(this.undoRedoArr.length > 0) {
      //   this.isRedoing = true
      //   this.canvas.add(this.undoRedoArr.pop())
      //   this.canvas.renderAll()
      // }
    ...
    updateUnRedoBtnStatus() {
      // if (!that.updateUnRedoStatus) return
      //撤销按钮状态更新
      ...
      // let btnStatus = {
      //   undoStatus: this.config.undoBtnStatus,
      //   redoStatus: this.config.redoBtnStatus
      // };
      // //更新主页面的btn状态
      // this.updateUnRedoStatus(btnStatus);
    },
    //更新是否是初始状态的状态
