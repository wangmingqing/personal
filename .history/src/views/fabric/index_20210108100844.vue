...
export default {
  ...
  data() {
    return {
      ...
      config: {
        ...
        undoCanvasState: [], // 存储各状态的json数据
        redoCanvasState: [], // 存储要恢复状态的json数据
        currentStateIndex: -1,
        undoStatus: false, // 撤销
        redoStatus: false, // 恢复
  ...
  methods: {
    ...
    updateCanvasState () {
      ...
      console.log(this.config.currentStateIndex + ' ' + this.config.canvasState.length - 1)
      if (this.config.currentStateIndex < this.config.canvasState.length - 1) {
        //恢复操作更新
        let oldData = JSON.parse(JSON.stringify(this.config.canvasState)) // 拷贝
        this.config.canvasState = oldData.splice(0, this.config.currentStateIndex + 1)
        console.log(this.config.canvasState)
    ...
    updateUnRedoBtnStatus() {
      //撤销按钮状态更新
      this.config.currentStateIndex > 0 ? this.$set(this.config, 'undoBtnStatus', true) : this.$set(this.config, 'undoBtnStatus', false)
      // 恢复按钮状态更新
      this.config.canvasState.length > this.config.currentStateIndex + 1 && this.config.canvasState.length > 1 ? 
      this.$set(this.config, 'redoBtnStatus', true) : this.$set(this.config, 'redoBtnStatus', false)
    },
    //更新是否是初始状态的状态
