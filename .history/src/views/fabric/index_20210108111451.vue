  ...
  methods: {
    init() {
      ...
      this.canvas.on('object:added', () => {
        this.config.ismodifyPlayer = false
        this.updateCanvasState()
        this.config.ismodifyPlayer = true
      })
      // 监听更新图层
      this.canvas.on('object:modified', () => {
        if(!this.config.ismodifyPlayer) return false
        this.updateCanvasState()
      })
      // 监听移除图层
    ...
    updateCanvasState () {
      ...
      this.config.isInitialStatus = this.config.canvasState.length <= 1 ? true : false
    },
    //更新撤销/回退的操作状态
